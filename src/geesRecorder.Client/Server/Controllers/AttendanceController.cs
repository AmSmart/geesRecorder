...
namespace geesRecorder.Client.Server.Controllers
{
    [ApiController]
    [Route("attendance")]
    public class AttendanceController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetEventDetails(int eventId)
        {
            ...
            var timeIns = personEvents.Where(x => x.TimeIn.HasValue).Select(x => (DateTime) x.TimeIn).OrderByDescending(x => x);
            var timeOuts = personEvents.Where(x => x.TimeOut.HasValue).Select(x => (DateTime) x.TimeOut).OrderByDescending(x => x);
